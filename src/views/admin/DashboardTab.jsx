...
export function DashboardTab({ bons, totalStock, totalUnavail, totalValue, materialCount, setCount, activeBons, overdueBons, reservedBons, recentLogs, onBonClick }) {
  return <div className="space-y-6">
    ...
    {overdueBons.length>0&&<div className="bg-red-50 border border-red-200 rounded-2xl px-5 py-4"><div className="flex items-start gap-3"><span className="text-lg">{"\u26a0\ufe0f"}</span><div><p className="font-semibold text-red-800 text-sm">Verlopen bonnen ({overdueBons.length})</p>{overdueBons.map(b=><p key={b.id} className="text-xs text-red-700 mt-1 cursor-pointer hover:underline" onClick={()=>onBonClick(b)}><span className="font-mono font-bold">{b.bon_number}</span> {"\u2014"} {b.user_name} {"\u2014"} {fmtDate(b.return_date)}</p>)}</div></div></div>}
    {activeBons.length>0&&<div><h3 className="text-sm font-bold text-gray-700 mb-3">Actieve bonnen</h3><div className="space-y-2">{activeBons.slice(0,10).map(b=><BonCard key={b.id} bon={b} onClick={()=>onBonClick(b)} showUser showAdminMark/>)}</div></div>}
