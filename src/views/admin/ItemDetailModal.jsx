...
export function ItemDetailModal({ detail, setDetail, bons, eq, addLog, getItemStats, onPrint, onEdit, onDelete, onOpenBon, onRegenBarcode, damageReports, onOpenDamage }) {
  return <Modal open={!!detail} onClose={()=>setDetail(null)} title="Materiaal" wide>
    {detail&&(()=>{
      const av = availQty(detail, bons);
      const stats = getItemStats(detail.id);
      const itemBons = bons.filter(b => b.status !== "completed" && (b.items || []).some(bi => bi.material_id === detail.id));
      // Ronde B blok 2: damage-tellingen per materiaal.
      const reportsForItem = (damageReports || []).filter(r => r.material_id === detail.id);
      const lostQty   = reportsForItem.filter(r => r.reason === "lost"   && r.status === "open").reduce((s, r) => s + r.quantity, 0);
      const brokenQty = reportsForItem.filter(r => r.reason === "broken" && r.status === "open").reduce((s, r) => s + r.quantity, 0);
      const totalReported = reportsForItem.length;
      const outOfService = detail.available_status === "out_of_service";
      ...
      return <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h3 className="font-bold text-gray-900 flex items-center gap-2 flex-wrap">
              ...
              {outOfService && <span className="text-[11px] font-semibold uppercase tracking-wide bg-red-100 text-red-700 px-1.5 py-0.5 rounded">buiten dienst</span>}
            </h3>
            <p className="text-sm text-gray-500">{detail.category} {"\u00b7"} {detail.stock} {detail.unit}</p>
          </div>
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between"><span className="text-gray-500">Beschikbaar</span><span className={`font-medium ${outOfService ? "text-red-600" : "text-emerald-600"}`}>{av}{outOfService ? " (buiten dienst)" : ""}</span></div>
        ...
        {/* Ronde B blok 2 — schade/verlies-samenvatting per materiaal */}
        {totalReported > 0 && <div className="bg-amber-50 rounded-xl border border-amber-100 p-4 text-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold text-amber-700 uppercase tracking-wide">Schade / verlies</p>
              <p className="text-amber-900 mt-1">
                {lostQty > 0 && <>{lostQty}x kwijt (open)</>}
                {lostQty > 0 && brokenQty > 0 && ", "}
                {brokenQty > 0 && <>{brokenQty}x kapot (open)</>}
                {lostQty === 0 && brokenQty === 0 && "Geen openstaande meldingen"}
              </p>
              <p className="text-xs text-amber-700 mt-0.5">{totalReported} melding{totalReported === 1 ? "" : "en"} totaal (open + historie)</p>
            </div>
            {onOpenDamage && <button onClick={() => { onOpenDamage(detail.id); setDetail(null); }} className="text-xs font-semibold text-amber-800 bg-amber-100 hover:bg-amber-200 rounded-lg px-3 py-1.5 flex-shrink-0">Bekijk</button>}
          </div>
        </div>}
