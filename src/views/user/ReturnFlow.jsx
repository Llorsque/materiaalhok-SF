...
export function ReturnFlow({ eq, sets, materialsLoading, materialsError, refreshMaterials, bons, refreshBons, setBonsError, user, onCancel, onDone }) {
  ...
  const findBonItemByScan = (code) => {
    ...
    const items = (activeBon.items || []).filter(isActiveItem);
    const matchMat = (eq || []).find((i) => i.barcode === code || i.barcode === upper);
    if (matchMat) {
      const bi = items.find((it) => it.material_id === matchMat.id && !it.returned);
      if (bi) return bi;
    }
    const matchSet = (sets || []).find((s) => s.barcode === code || s.barcode === upper);
    if (matchSet) {
      const bi = items.find((it) => it.set_id === matchSet.id && !it.returned);
      if (bi) return bi;
    }
    const numId = parseInt(code, 10);
    if (!Number.isNaN(numId)) {
      const bi = items.find((it) => (it.material_id === numId || it.set_id === numId) && !it.returned);
      if (bi) return bi;
    }
  ...
  const openItems = (activeBon?.items || []).filter((it) => isActiveItem(it) && !it.returned);

  const selectAllOpen = () => {
    setSelectedItems(new Set(openItems.map((it) => it.id)));
  };
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50">
    ...
    <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
        ...
        {(activeBon.items||[]).filter(isActiveItem).map((bi) => {
